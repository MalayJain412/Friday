...
def get_conversation_log_path():
    global conversation_log_path
    if conversation_log_path is None:
        log_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "KMS", "logs")
        os.makedirs(log_dir, exist_ok=True)
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        conversation_log_path = os.path.join(log_dir, f"conversation_{timestamp}.txt")
    ...
    def _process_stream_event(self, data: dict) -> None:
        ...
        # for more information about the different types of events
            ...
            if len(alts) > 0 and alts[0].text:
                # Log transcript (interim and final)
                self._log_stt_output(alts[0].text)
